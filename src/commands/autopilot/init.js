const { TwilioClientCommand } = require("@twilio/cli-core").baseCommands,
  {
    ...
    renameFile,
    updateSchemaFile,
  } = require("../../lib/serverless/create-files"),
  {
    installDependencies,
  } = require("../../lib/serverless/install-dependencies"),
  ora = require("ora"),
  path = require("path"),
...
class InitAssistant extends TwilioClientCommand {
  async run() {
    ...
    try {
      let { flags } = await this.parse(InitAssistant),
        clonedAssistant = "";
      ...
      await createPackageJSON(
        fullPath,
        camelCase(clonedAssistant).toLowerCase()
      );
      spinner.succeed();
...
InitAssistant.flags = Object.assign(convertYargsOptionsToOclifFlags(options), {
