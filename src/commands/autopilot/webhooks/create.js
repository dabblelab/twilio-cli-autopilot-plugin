...
class CreateAssistantWebhook extends TwilioClientCommand {
  async run() {
    ...
    let { flags } = await this.parse(CreateAssistantWebhook);
    ...
    const { assistantSid, webhookUniqueName, events, webhookURL, method } =
      flags;
    ...
    const mEvents = eventList.filter((l) =>
    ...
    try {
      ...
      const webhook = await AutopilotCore.webhooks.create(
        this.twilioClient,
        assistantSid,
        params
      );
