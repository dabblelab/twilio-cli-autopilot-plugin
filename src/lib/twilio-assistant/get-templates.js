...
const clone = async (url, templateName, inquirer) => {
    ...
    try{
        ...
        const template_name = await getTargetTemplateName(templateList, templateName, inquirer);
...
const gitCloneTemplate = async(cloneTemplate) =>{
...
const deleteExistCloneDir = async(cloneDir) => {
    
    if(fs.existsSync(cloneDir)){

        try{

            const clonedFolder = fs.readdirSync(cloneDir);
            for(var file in clonedFolder)
            {
                
                if(fs.statSync(path.join(cloneDir, clonedFolder[file])).isDirectory())
                {
                    fs.removeSync(path.join(cloneDir, clonedFolder[file]));
                }
                else
                {
                    fs.unlinkSync(path.join(cloneDir, clonedFolder[file]))
                }
            }
        }catch(error){
        }
